...
static const struct fs_parameter_spec btrfs_fs_parameters[] = {
	...
	fsparam_flag("crash_post_commit", Opt_crash_post_commit),
#endif
	{}
};
...
"'norecovery' is for compatibility only, recommended to use 'rescue=nologreplay'");
	...
	case Opt_skip_extent_writes:
		...
		break;
	case Opt_crash_post_commit:
		btrfs_set_opt(ctx->mount_opt, CRASH_POST_COMMIT);
		break;
#endif
	default:
...
static int btrfs_show_options(struct seq_file *seq, struct dentry *dentry)
{
	...
	if (btrfs_test_opt(info, CRASH_POST_COMMIT))
		seq_printf(seq, ",crash_post_commit");
...
again:
	...
	fs_info->super_backup = kzalloc(BTRFS_SUPER_INFO_SIZE, GFP_KERNEL);
