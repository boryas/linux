...
static noinline void switch_commit_roots(struct btrfs_trans_handle *trans)
{
	...
	list_for_each_entry_safe(root, tmp, &cur_trans->switch_commits,
		...
		set_dirty_list_linkage(root, NULL, __func__);
			       ...
			       struct btrfs_root *root)
{
	...
	printk(KERN_INFO "BO: update cowonly root (2) " BTRFS_KEY_FMT " (%px)\n", BTRFS_KEY_FMT_VALUE(&root->root_key), root);
	while (1) {
		...
		printk(KERN_INFO "BO: update cowonly root (3) " BTRFS_KEY_FMT " (%px)\n", BTRFS_KEY_FMT_VALUE(&root->root_key), root);
...
again:
	...
	while (!list_empty(&fs_info->dirty_cowonly_roots)) {
		...
		set_dirty_list_linkage(root, &trans->transaction->switch_commits, __func__);
		list_move_tail(&root->dirty_list,
			       &trans->transaction->switch_commits);

		printk(KERN_INFO "BO: update cowonly root (1) " BTRFS_KEY_FMT " (%px)\n", BTRFS_KEY_FMT_VALUE(&root->root_key), root);
...
int btrfs_commit_transaction(struct btrfs_trans_handle *trans)
{
	...
	set_dirty_list_linkage(fs_info->tree_root, &cur_trans->switch_commits, __func__);
	list_add_tail(&fs_info->tree_root->dirty_list,
		      &cur_trans->switch_commits);

	btrfs_set_root_node(&fs_info->chunk_root->root_item,
			    fs_info->chunk_root->node);
	set_dirty_list_linkage(fs_info->chunk_root, &cur_trans->switch_commits, __func__);
	...
	if (btrfs_fs_incompat(fs_info, EXTENT_TREE_V2)) {
		...
		set_dirty_list_linkage(fs_info->block_group_root, &cur_trans->switch_commits, __func__);
		if (list_empty(&fs_info->block_group_root->dirty_list))
			list_add_tail(&fs_info->block_group_root->dirty_list,
				      &cur_trans->switch_commits);
		else
			list_move_tail(&fs_info->block_group_root->dirty_list,
				       &cur_trans->switch_commits);
	}
