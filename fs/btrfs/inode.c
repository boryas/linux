...
static int extent_range_clear_dirty_for_io(struct btrfs_inode *inode, u64 start, u64 end)
{
	...
	for (pgoff_t index = start >> PAGE_SHIFT; index <= end_index; index++) {
		 ...
		 * and checksums the folio contents into the compression buffer;
		 * this is the one checksum path that does not go through
		 * extent_writepage_io().  Keep the generic dirty flag
		 * (btrfs_folio_clamp_clear_dirty() below clears the btrfs dirty
		 * state).  The folios are locked by the caller.
		 */
		folio_mkclean(folio);
